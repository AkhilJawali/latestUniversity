...
// A4-340 section 5.3 — CRUD hooks for session-derivation rules, against the real
// A4-390 backend route: /api/v1/scheduling-config/derivation-rules
// (DerivationRuleController @RequestMapping). apiClient baseURL is /api/v1.
const BASE = '/scheduling-config/derivation-rules';

const keys = {
  list: (campusId) => ['scheduling-config', 'derivation-rules', 'list', campusId],
};
...
export function useUpdateDerivationRule(campusId) {
  const qc = useQueryClient();
  return useMutation({
...
export function useDeleteDerivationRule(campusId) {
  const qc = useQueryClient();
  return useMutation({
